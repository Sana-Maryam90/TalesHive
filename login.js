...
profileicon.addEventListener("click", () => {
  if (loggedIn) {
    ...
    showAlert(
      alert,
      "Please login to access your profile. Or Register if you don't have an account"
    );
    wrapper.classList.add("active-popup");
  }
});

const writelogin = document.querySelector(".write-login");
writelogin.addEventListener("click", () => {
  const useridParam = urlParams.get("userid");
  if (loggedIn) {
    location.replace("draft.html");
    header("Location: draft.html?userid=useridParam");
  } else {
    showAlert(
      alert,
      "Please login to write stories. Or Register if you don't have an account"
    );
    wrapper.classList.add("active-popup");
  }
});
